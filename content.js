...
(function() {
  ...
  async function addReviewersAutomatically(reviewersList = null) {
    ...
    console.log('GitLab Auto Reviewer: 드롭다운 버튼 발견', dropdownToggle);
    ...
    for (const reviewer of reviewers) {
      ...
      for (let attempt = 1; attempt <= 5 && !success; attempt++) {
        if (attempt > 1) {
          console.log(`GitLab Auto Reviewer: "${reviewer}" ${attempt}번째 시도`);
        }
        ...
        if (firstResult) {
          ...
          console.error(`GitLab Auto Reviewer: "${reviewer}" 검색 결과 없음 (시도 ${attempt}/5)`);
          await new Promise(resolve => setTimeout(resolve, 500));
        }
      }

      if (!success) {
        console.error(`GitLab Auto Reviewer: "${reviewer}" 5번 시도 후 실패`);
      }
    }
